...
class Player(pygame.sprite.Sprite):
    def __init__(self, pos, groups, health=10):
        ...
        self.image = PLAYER_IDLE[0].convert_alpha()                                                                          # assign image to the player # convert_alpha() function used to specify that the image should be rendered with alpha colors (for .png format).
    ...
    def input(self):
        ...
        if keys[pygame.K_UP]:
            ...
            self.image = pygame.transform.scale(PLAYER_IDLE[2].convert_alpha(), self.new_size_image)
        if keys[pygame.K_DOWN]:
            keys_input_vector.y += 1
            self.current_direction = 'down'
            self.image = pygame.transform.scale(PLAYER_IDLE[1].convert_alpha(), self.new_size_image)
        if keys[pygame.K_LEFT]:
            keys_input_vector.x -= 1
            self.current_direction = 'left'
            self.image = pygame.transform.scale(PLAYER_IDLE[3].convert_alpha(), self.new_size_image)
        if keys[pygame.K_RIGHT]:
            keys_input_vector.x += 1
            self.current_direction = 'right'
            self.image = pygame.transform.scale(PLAYER_IDLE[4].convert_alpha(), self.new_size_image)
